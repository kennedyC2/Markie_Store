...
const AdminTorch = (props) => {
    ...
    const updateState = (pos, id) => {
        const _data = {
            ...
            id: id,
            details: {
                title: penTorch[pos]["title"],
                quantity: penTorch[pos]["quantity"],
                price: penTorch[pos]["price"],
                colors: penTorch[pos]["colors"],
                category: penTorch[pos]["category"],
                description: penTorch[pos]["description"],
                misc: penTorch[pos]["misc"],
            },
            images: {
                main: penTorch[pos]["images"]["main"],
                image_1: penTorch[pos]["images"]["image_1"],
                image_2: penTorch[pos]["images"]["image_2"],
            },
        }
        setData(_data)
    }
                        ...
                        ? penTorch.map((item, index) => {
                            return (
                                <div key={index} className="card" id={item._id + "_" + index}>
                                    <img src={domain + "image/" + item.images.main} className="card-img-top" alt="penTorch" width={"100%"} height={"100%"} />
                                    <div className="card-body">
                                        ...
                                        <div className="card-body d-flex justify-content-between">
                                            <button type="button" className="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target={"#penTorch_" + index + "a"}>
                                                view
                                            </button>

                                            <div className="modal fade" id={"penTorch_" + index + "a"} data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby={"penTorch_" + index + "a_Label"} aria-hidden="true">
                                                <div className="modal-dialog modal-dialog-centered modalThree">
                                                    <div className="modal-content">
                                                        <div className="modal-header">
                                                            <h1 className="modal-title fs-5 invisible" id={"penTorch_" + index + "a_Label"}>Modal title</h1>
                                            ...
                                            <button type="button" className="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target={"#penTorch_" + index + "b"}>
                                                delete
                                            </button>

                                            <div className="modal fade" id={"penTorch_" + index + "b"} data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby={"penTorch_" + index + "b_Label"} aria-hidden="true">
                                                <div className="modal-dialog modal-dialog-centered modalThree">
                                                    <div className="modal-content">
